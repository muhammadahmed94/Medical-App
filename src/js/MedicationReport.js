...
import { getSettingsItem } from './utils/utils';
...
class MedicationReport extends Component {
    ...
    componentDidMount() {
        ...
        payloadForNote = null;
        noteAttached = false;
        const loggedInUserRole = !!store.getState().auth ? store.getState().auth.userDetails.roles[0] : "";
        ...
        if (assignedToTypeId) {
            ...
            stateObject.note = ''; //handler1
            stateObject.value = assignedToTypeId === 3 ? 'DIETITIAN' : 'DOCTOR';
        ...
        if (sessionStorage.getItem('sendingReportTo')) {
            stateObject.value = sessionStorage.getItem('sendingReportTo');
        }
        if (this.props.location && this.props.location.state && this.props.location.state.noteData) {
            noteAttached = true;

            stateObject.displayNotes = this.props.location.state.noteData;
            stateObject.showMyNote = true;

            payloadForNote = { reportNotes: [] };
            payloadForNote.reportNotes = this.props.location.state.noteData;
        }
        GetMedications(this.props.match.params.id).then((response) => {

            const finalData =
                this.props.location && this.props.location.state && this.props.location.state.finalData
                    ? this.props.location.state.finalData
                    : null;
            if (response && loggedInUserRole === 'ROLE_DOCTOR' && !isEmpty(finalData)) {
                response = [...response, finalData.medications]; // setting current medications when creates a report..
            }
            this.setState({
        ...
        BGLGetResource(this.props.match.params.id).then((response) => {
            ...
            if (response &&
                ...
                response = [...response, { bglRanges: suggestedBGLRanges.bglRanges }]; // setting current medications when creates a report..
            }
            this.setState({ patientRanges: response });
        ...
        if (this.props.location.state) {
            ...
            if (reportId) {
                GetSingleReport(reportId).then((response) => {

                    const activeTab =
                        !isEmpty(response) &&
                            !isEmpty(response.suggestedMedication) &&
                            !isEmpty(response.suggestedMedication.medications)
                            ? 1
                            : 2;
                    const activeTab1 =
                        !isEmpty(response) && !isEmpty(response.suggestedBgl) && !isEmpty(response.suggestedBgl.bglRanges)
                            ? 1
                            : 2;
                    if (!response.read &&
                        this.props &&
                        ...
                        response.createdByRole === 'ROLE_DOCTOR' &&
                        !response.snapshot) {
                        MarkReportAsApproved(
                            { read: true, reportId: reportId },
                            response => {
                                console;
                            },
                            err => {
                    ...
                    this.setState(
                        {
                            ...
                            createdByRole: response.createdByRole,
                            createdByName: response.createdByName,
                            reportLocked: 'lockedById' in res,
                            canEditReport: response.status == 8 || response.status == 5 || response.status == 10,
                            snapshotReport: response.snapshot, // first make sure that there is already something in the report that can be edited
                        },
                        () => {

                            this.setState({
                                reportExists: true, assignedToName: response.assignedToName
                                , assignedToTypeId: response.assignedToTypeId, assignedToRole:
                                    response.assignedToTypeId === 1
                                        ? 'ROLE_DOCTOR'
                                        : response.assignedToTypeId === 2
                                            ? 'ROLE_EDUCATOR'
                                            : 'ROLE_DIETICIAN',
                                createdByName: response.createdByName,
                                createdByRole: response.createdByRole,
                                timeOfReportCreation: moment(response.createdAt).format(YY_MM_DD),
                                timeOfReportUpdate: moment(response.updatedAt).format(YY_MM_DD)
                            });
                            if (
                                this.props &&
                                this.props.location &&
                ...
                let activeTab1 =
                    this.props.location &&
                        ...
                        : 2;
                if (activeTab === 1)
                    this.setState({ isSuggestedMedicationUpdated: true, isSuggestedMonitoringUpdated: true, showPrompt: true });
            ...
            if (this.props.location.state.from === 'dashboard') {
                if (isEnglishLanguageCode(lang)) {
                    this.tabs = CONSTANTS.TABS.ENGLISH.MAIN_TAB;
                    this.tabs1 = CONSTANTS.TABS.ENGLISH.TAB_1;
                } else {
                    this.tabs = this.tabs = CONSTANTS.TABS.ARABIC.MAIN_TAB;
                    this.tabs1 = CONSTANTS.TABS.ARABIC.TAB_1;
                }
            } else {
                const config = {
                    textMargin: 'ml-3',
                ...
                if (isEnglishLanguageCode(lang)) {
                    ...
                    this.tabs = CONSTANTS.TABS.ARABIC.MAIN_TAB;
                    this.tabs1 = CONSTANTS.TABS.ARABIC.TAB_1;
                }
            }
        } else {
        ...
        if (!(reportId && reportId !== null && reportId !== '')) {
            const selectedReportHCP =
                this.props.location &&
                    ...
                    ? this.props.location.state.selectedReportHCP
                    : '';
            stateObject.value = selectedReportHCP || (stateRole === 'ROLE_EDUCATOR' ? 'DOCTOR' : 'DIETITIAN');
        }
        this.setState({ ...this.state, stateObject });
    }
    ...
    render() {
        ...
        const lang = localStorage.getItem('i18nextLng');
        let config = Styles.NormalLanguageCodeStyles;

        if (isArabicLanguageCode(lang)) {
            config = Styles.ArabicLanguageCodeStyle
        }
            ...
            this.props.location.state && this.props.location.state.finalData
                ...
                : suggestedMedication
                    ...
                    : reportData.suggestedMedication
                        ? reportData.suggestedMedication
                        : null;
        console.log('SUGGSTED MEDICATIONS', suggestedMedication);
        let currentData;
        if (this.state.reportExists) {
        ...
        if (this.haveFullAccessRights(role)) {
            // var self = this;
            const lang = localStorage.getItem('i18nextLng');
            const config = {
                textMargin: 'ml-3',
                ...
                <I18n ns="translations">
                    {t => (
                        <button
                            ...
                            onClick={this.getMedButtonEvent(reportId)}
                        >
                            <Icon type="edit" />
                            <span className={`${config.textMargin}`}>{t('EDIT')}</span>
                        </button>
                ...
                <I18n ns="translations">
                    {t => (
                        <button
                            ...
                            onClick={this.bglEditEvent(reportId, currentPatientName, currentPatientDetails, currentData)}
                        >
                            <Icon type="edit" />
                            <span className={`${config.textMargin}`}>{t('EDIT')}</span>
                        </button>
        ...
        } else if (this.secondAccessRights(role)) {
                ...
                <I18n ns="translations">
                    {t => (
                        <button
                            ...
                            onClick={this.viewMedicationEvent(role)}
                        >
                            <Icon type="eye-o" />
        ...
        if (!this.state.reportExists) {
            ...
            assignedToRole = getRole(this.state.assignedToRole);
        }
    ...
    //Methods Added TO clean Expressions AND event Methods In Code
    getMedButtonEvent = (reportId) => {
    ...
    bglEditEvent = (reportId, currentPatientName, currentPatientDetails, currentData) => {
...
/*this.props.location.state.role !== 'ROLE_DOCTOR' &&*/ this.props.location
    ...
    haveFullAccessRights = (role) => {
        return (!this.state.snapshotReport &&
            (this.state.canEditReport ||
                (this.state.reportLocked &&
                    this.state.reportData.lockedById &&
                    this.state.reportData.lockedById == this.state.loggedInUserId)) &&
...
const Styles = {
    ArabicLanguageCodeStyle:
    {
        textMargin: 'mr-3',
        btnDirection: 'float-left',
        btnMargin: 'ml-md-2',
        btnDirectionType2: 'text-left',
        headingMargin: 'mr-0',
        textDirection: 'text-right',
        buttonPadding: 'pr-0',
        iconPadding: 'pl-2',
    },
    NormalLanguageCodeStyles: {
        textMargin: 'ml-3',
